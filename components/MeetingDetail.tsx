  ...
  return (
    <div className="bg-white p-6 sm:p-10 max-w-3xl mx-auto rounded-xl shadow-sm border border-gray-200 print:shadow-none print:border-none print:p-0">
      ...
      <div className="text-center mb-8 border-b pb-6 print:border-b-2 print:border-black">
        <h1 className="text-3xl font-serif text-gray-900 mb-2">Sacrament Meeting</h1>
        <p className="text-gray-600 text-lg">{formattedDate}</p>
        <span className="inline-block mt-2 bg-gray-100 text-gray-700 text-xs px-3 py-1 rounded-full uppercase tracking-wider font-semibold print:hidden">
          {meeting.meetingType}
        </span>
      </div>
      ...
      <div className="space-y-4 text-gray-800 text-lg">
        ...
        <div className="flex justify-between mt-6 pt-4 border-t border-dashed">
          ...
          <span className="text-right">
            {meeting.openingHymn.title} #{meeting.openingHymn.number}
          </span>
        </div>
        <div className="flex justify-between">
        ...
        {meeting.wardBusiness && meeting.wardBusiness.length > 0 && (
          <div className="mt-6 pt-4 border-t border-dashed">
            <h3 className="font-bold text-gray-900 mb-2">
              Ward Business {meeting.stakeBusiness && '(Includes Stake Business)'}
            </h3>
            <ul className="list-disc pl-6 text-base text-gray-700 space-y-1">
        ...
        <div className="flex justify-between mt-6 pt-4 border-t border-dashed">
          ...
          <span className="text-right">
            {meeting.sacramentHymn.title} #{meeting.sacramentHymn.number}
          </span>
        </div>
          ...
          {meeting.speakers && meeting.speakers.length > 0 ? (
              ...
              {meeting.speakers.map((speaker, i) => (
                <li key={i} className="flex flex-col text-center">
                  ...
                  <span className="text-base text-gray-600 italic">
                    {speaker.topic}{' '}
                    {speaker.type === 'musical-number' ? '(Special Musical Number)' : ''}
                  </span>
                </li>
              ))}
            </ul>
            ...
            <p className="text-center text-gray-500 italic">
              {meeting.meetingType === 'testimony'
                ? 'Bear testimonies as directed by the Spirit.'
        ...
        <div className="flex justify-between mt-8 pt-4 border-t border-dashed">
          ...
          <span className="text-right">
            {meeting.closingHymn.title} #{meeting.closingHymn.number}
          </span>
        </div>
        <div className="flex justify-between">
      ...
      <div className="mt-12 text-center print:hidden">
        <button
          ...
          className="bg-gray-800 text-white px-6 py-2 rounded-md shadow hover:bg-gray-700 transition font-medium"
        >
          Print Program
        </button>
      </div>
